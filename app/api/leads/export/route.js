...
export async function GET(request) {
  ...
  try {
    ...
    const rows=leads.map(l=>[l._id,`"${l.fullName}"`,l.email,l.phone,l.licenseType,l.homeCountryLicense,`"${(l.emirates||[]).join(';')}"`,l.status,`"${(l.additionalInfo||'').replace(/"/g,'""')}"`,l.createdAt].join(',')).join('\n');
    return new Response(header+rows,{ headers:{ 'Content-Type':'text/csv', 'Content-Disposition':`attachment; filename="leads-${Date.now()}.csv"` } });
