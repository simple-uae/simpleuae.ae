...
export async function PUT(request) {
  ...
  try {
    ...
    if (!newKey||newKey.length<8) return NextResponse.json({ success:false, message:'Key must be at least 8 characters' },{ status:400 });
    const hash = await bcrypt.hash(newKey, 10);
    await Settings.findOneAndUpdate({ key:'admin_key_hash' },{ $set:{ value:hash, sensitive:true } },{ upsert:true });
