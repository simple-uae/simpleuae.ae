...
export async function GET(request,{ params }) {
  try { const { id }=await params; const school=await School.findById(id); if(!school) return NextResponse.json({ success:false, message:'School not found' },{ status:404 }); return NextResponse.json({ success:true, data:school }); }
...
export async function DELETE(request,{ params }) {
  const authErr=requireAdmin(request); if(authErr) return authErr;
  try {
    const { id }=await params; const school=await School.findByIdAndDelete(id);
    if(!school) return NextResponse.json({ success:false, message:'School not found' },{ status:404 });
